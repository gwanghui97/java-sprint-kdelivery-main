...
public class KDeliveryMain {
  ...
  public void selectAddShopMenu() {
    ...
    if (currentIndex != -1) {
      // shops를 순회하며 해당 가게가 존재하는지 찾아야 함
      // shopName == 기존의 shop 객체의 shopName이 일치하지 않을때만 새로 생성
      // 이외의 경우에는 기존의 Shop에 추가
//      for (int i = 0; i < currentIndex; i++) {
//        if (shops[i].getShopName().equals(shopName)) {
//          Shop existShop = shops[i];
